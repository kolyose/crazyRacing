...
export default function (data){
        return new Promise((resolve, reject) => {
            ...
            while (getRemainingDistanceSum() > 0){
                //1st STAGE:
            ...
            function shiftOrMovePlayer(playerId, racetrackIndex, slotIndex){
                    ...
                    && slotsPerRacetrack[targetRacetrackIndex][slotIndex] === null)
                {
                    ...
                    milestonesByPlayerId[playerId].push({x: slotIndex, y:racetrackIndex, s:(data[playerId].actions.boost ? 2 : 1)})
                    milestonesByPlayerId[playerId].push({x: slotIndex, y:targetRacetrackIndex, s:(data[playerId].actions.boost ? 2 : 1)})
                ...
                else
                {
                    ...
                    if (!slotsPerRacetrack[racetrackIndex][targetSlotIndex]){
                        ...
                        milestonesByPlayerId[playerId].push({x: targetSlotIndex, y:racetrackIndex, s:(data[playerId].actions.boost ? 2 : 1)})
                    }
                }
