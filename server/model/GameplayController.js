...
export default (function() {
  ...
  const _superpoweredPlayerIds = new WeakMap();
  ...
  class GameplayController extends EventEmitter {
    constructor(room) {
      ...
      _superpoweredPlayerIds.set(this, {});
    ...
    init() {
      ...
      for (let client of this.room.clients) {
        ...
        this._setPositionByPlayerId(client.playerVO.id, {
          x: 0,
          y: Util.getRandomValueFromArray(raceTrackNumbers),
        });
        ...
        client.socket.on(PLAYER_ACTIONS, async data => {
          ...
          //preventing superpowered players from superpowering again
          if (this._checkIfPlayerAppliedSuperpower(playerId)) {
            playerActions.superpower = null;
          }

          //and memorizing player applied superpower
          if (playerActions.superpower) {
            this._addSuperpoweredPlayerId(playerId);
          }
    ...
    _getRoundResultsData(computedResults) {
      ...
      for (let playerId in computedResults)
        if (hasOwnProperty.call(computedResults, playerId)) {
    ...
    _checkIfPlayerAppliedSuperpower(playerId) {
      let result = _superpoweredPlayerIds.get(this)[playerId] !== undefined;
      return result;
    }

    _addSuperpoweredPlayerId(id) {
      const ids = _superpoweredPlayerIds.get(this);
      ids[id] = id;
      _superpoweredPlayerIds.set(this, ids);
    }
